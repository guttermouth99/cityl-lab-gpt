...
import { getActiveJobs, markJobAsSent, hasJobBeenSentToUser } from '@baito/db/queries'
...
export const sendBatchTask = task({
  ...
  run: async (payload: {
    ...
    console.log(`Processing batch ${batchIndex} with ${users.length} users (${type})`)
      ...
      try {
        ...
        if (type === 'daily') {
          await sendJobAlertEmail(user.email, {
            userName: user.name,
            alertName: user.alerts.length === 1 ? user.alerts[0]!.name : 'Your Alerts',
