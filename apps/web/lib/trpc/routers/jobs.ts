import { z } from 'zod'
import { router, publicProcedure, customerProcedure, adminProcedure } from '../trpc'
...
export const jobsRouter = router({
  ...
  byOrganization: publicProcedure
    .input(z.object({ organizationId: z.string(), limit: z.number().default(20) }))
    .query(async ({ input }) => {
      return jobQueries.getJobsByOrganization(input.organizationId, input.limit)
    }),
