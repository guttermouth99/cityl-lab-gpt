...
export async function updateUserAlertFrequency(userId: string, frequency: AlertFrequency) {
...
export async function updateUserRole(userId: string, role: 'user' | 'customer' | 'admin') {
  const [updated] = await db
    .update(users)
    .set({ 
      role,
...
// Track sent jobs
export async function markJobAsSent(userId: string, jobId: string, alertId?: string | null) {
  const id = crypto.randomUUID()
...
export async function hasJobBeenSentToUser(userId: string, jobId: string) {
  const result = await db.query.sentJobs.findFirst({
    where: and(
      eq(sentJobs.userId, userId),
      eq(sentJobs.jobId, jobId),
    ),
  })
