...
function g() {
  ...
  function r(a) {
    ((o) => {
      var t = /\((?:[^()]|\((?:[^()]|\([^()]*\))*\))*\)/.source,
        e = /(?:\b\w+(?:<braces>)?|<braces>)/.source.replace(
          /<braces>/g,
          () => t
        ),
        n = (o.languages.pascaligo = {
          ...
          "class-name": [
            ...
            {
              pattern: RegExp(
                /<type>(?=\s+is\b)/.source.replace(/<type>/g, () => e),
                "i"
              ),
              inside: null,
            },
            {
              pattern: RegExp(
                /(:\s*)<type>/.source.replace(/<type>/g, () => e)
              ),
              lookbehind: !0,
              inside: null,
            },
        ...
        p = ["comment", "keyword", "builtin", "operator", "punctuation"].reduce(
          (i, c) => ((i[c] = n[c]), i),
          {}
        );
      n["class-name"].forEach((i) => {
        i.inside = p;
      });
    })(a);
  }
...
var l = g();
const d = b(l),
  y = f({ __proto__: null, default: d }, [l]);
export { y as p };
