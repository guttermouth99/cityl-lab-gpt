...
class CompositeExporter {
  ...
  export(spans, resultCallback) {
    ...
    const filteredSpans = spans.filter((span) => {
      ...
      return !(
        (span.instrumentationScope?.name?.startsWith("@opentelemetry") &&
  ...
  forceFlush() {
    return Promise.all(
      this.exporters.map((e) => e.forceFlush?.() ?? Promise.resolve())
    ).then(() => void 0);
  }
}
...
function getSampler(config) {
  ...
  switch (config.sampling.type) {
    ...
    case "always_off":
      return new AlwaysOffSampler();
    case "parent_based": {
      const rootSampler = new TraceIdRatioBasedSampler(
        config.sampling.root?.probability || 1.0
      );
  ...
  if (config.export?.type === "otlp") {
    if (config.export?.protocol === "grpc") {
      ...
      const exporterEndpoint =
        config.export.endpoint ?? process.env.OTEL_EXPORTER_OTLP_ENDPOINT;
